-- 20260922140001_drop_profile_equipment_extras.sql
-- REVERSE of 20260922140000_add_profile_equipment_extras.sql
...
-- undo 20260922140000, after review.
...
--
-- Drops public.profiles.equipment_extras. Catalog-only. No data write against
-- any other column. Existing profile values (including equipment) are left
-- untouched.
--
...
--   equipment_extras exists as text[], nullable, no DEFAULT
--     -> DROP COLUMN
--   equipment_extras is already absent
...
BEGIN
  ...
  IF n_col = 1 THEN
    ...
    IF rec.atttypid IS DISTINCT FROM textarr_oid THEN
      RAISE EXCEPTION 'precondition failed: public.profiles.equipment_extras has type %, expected text[]; refusing to drop a different column',
        rec.atttypid;
    END IF;
    IF rec.attnotnull THEN
      RAISE EXCEPTION 'precondition failed: public.profiles.equipment_extras is NOT NULL; refusing to drop an unexpected definition';
    END IF;
    IF rec.atthasdef OR rec.def IS NOT NULL THEN
      RAISE EXCEPTION 'precondition failed: public.profiles.equipment_extras has a DEFAULT (%); refusing to drop an unexpected definition',
        rec.def;
    END IF;
  END IF;
END
...
BEGIN
  ...
  IF n_col <> 0 THEN
    RAISE EXCEPTION 'postcondition failed: public.profiles.equipment_extras still exists';
  END IF;
