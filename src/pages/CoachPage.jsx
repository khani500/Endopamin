...
const COACHES = [
  {
    id: 'aria', name: 'Aria', role: 'Hypertrophy & Muscle Science', color: '#CCFF00',
    unlocked: true, tagline: 'Evidence-based. Volume-driven. Results you can measure.',
  ...
  {
    id: 'kane', name: 'Kane', role: 'Strength & Power', color: '#FFA53C',
    unlocked: true, tagline: 'Heavy. Hard. Progressive. No shortcuts.',
  ...
  {
    id: 'blaze', name: 'Blaze', role: 'CrossFit & Fat Loss', color: '#FF6B6B',
    unlocked: true, unlockReq: 'Level 5 — 3000 XP', unlockXp: 3000,
    tagline: 'High intensity. Maximum burn. Zero excuses.',
  ...
  {
    id: 'nova', name: 'Nova', role: 'Functional & Bodyweight', color: '#A064FF',
    unlocked: true, unlockReq: '7-day streak',
    tagline: 'Master your body. No equipment needed.',
  ...
  {
    id: 'zara', name: 'Zara', role: 'Recovery & Rehab', color: '#C084FC',
    unlocked: true, unlockReq: '10 workouts completed',
    tagline: 'Move pain-free. Recover smarter. Come back stronger.',
...
const ATHLETE_STATUSES = [
  { id: 'fired', label: 'Fired', svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-full h-full"><path d="M12 2c0 0-5 4-5 9a5 5 0 0010 0c0-5-5-9-5-9z"/><path d="M12 12c0 0-2 1.5-2 3a2 2 0 004 0c0-1.5-2-3-2-3z" fill="currentColor"/></svg> },
  { id: 'strong', label: 'Strong', svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-full h-full"><path d="M6 4h2l1 3h6l1-3h2"/><path d="M9 7v10M15 7v10"/><path d="M6 20h12"/><path d="M8 11h8"/></svg> },
  { id: 'tired', label: 'Tired', svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-full h-full"><circle cx="12" cy="12" r="9"/><path d="M8 15s1.5-1 4-1 4 1 4 1"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/><path d="M8 9.5c.5-.5 1-.5 1.5 0M14.5 9.5c.5-.5 1-.5 1.5 0"/></svg> },
  { id: 'calm', label: 'Calm', svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-full h-full"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"/><path d="M8 14s1.5 2 4 2 4-2 4-2"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/></svg> },
  { id: 'hyped', label: 'Hyped', svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-full h-full"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg> },
];
...
export default function CoachPage() {
  ...
  const [athleteStatus, setAthleteStatus] = useState('strong');
    ...
    <main className="h-[100dvh] bg-[#080808] text-white flex flex-col pb-20 overflow-hidden">
      ...
      {view === 'chat' && (
        <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
          ...
          {/* Athlete Status */}
          <div className="mx-5 mb-3 grid grid-cols-5 gap-2 flex-shrink-0">
            {ATHLETE_STATUSES.map(status => (
              <button
                key={status.id}
                type="button"
                onClick={() => setAthleteStatus(status.id)}
                className="rounded-[12px] py-2 text-center transition-all active:scale-95 border"
                style={{
                  background: athleteStatus === status.id ? `${coach.color}15` : 'rgba(255,255,255,0.03)',
                  borderColor: athleteStatus === status.id ? `${coach.color}40` : 'rgba(255,255,255,0.07)',
                }}
              >
                <div
                  className="w-4 h-4 mx-auto mb-1"
                  style={{ color: athleteStatus === status.id ? coach.color : 'rgba(255,255,255,0.6)' }}
                >
                  {status.svg}
                </div>
                <p
